...
if [ -z "$workflow_name" ]; then
    echo "Usage: $0 <workflow-name> <--all | --case test_case_name> [--skip-setup] [--local] [--quiet|-q]"
    echo "Examples:"
    echo "  $0 <workflow-name> --case my-test    # Run single test case"
    echo "  $0 <workflow-name> --all              # Run all test cases"
    echo "Options:"
...
run_single_test_case() {
    ...
    [ "$quiet" = false ] && echo "Command: $cmd" && echo ""
    ...
    if [ "$error_value" != "null" ] || [ $cmd_exit_code -ne 0 ]; then
        ...
        [ "$error_value" != "null" ] && echo "  Error: $(jq -r '.error' "$result_json")"
...
if [ "$run_all" = true ]; then
    ...
    if [ ${#passed_cases[@]} -gt 0 ]; then
        ...
        for case in "${passed_cases[@]}"; do echo "  - $case"; done
        echo ""
    fi

    if [ ${#failed_cases[@]} -gt 0 ]; then
        echo "✗ Failed test cases:"
        for case in "${failed_cases[@]}"; do echo "  - $case"; done
        echo ""
        exit 1
    fi
